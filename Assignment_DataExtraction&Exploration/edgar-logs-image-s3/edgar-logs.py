...
# Create logfile.
logfile_name = "edgar-logs-logfile.txt"
...
if edgar_logs_config.aws_access_key_id is None or edgar_logs_config.aws_access_key_id == "":
    log_entry("Required parameter 'edgar_logs_config.aws_access_key_id' is missing: check the edgar_logs_config.py file")
    sys.exit()

if edgar_logs_config.aws_secret_access_key is None or edgar_logs_config.aws_secret_access_key == "":
    log_entry("Required parameter 'edgar_logs_config.aws_secret_access_key' is missing: check the edgar_logs_config.py file")
    sys.exit()

if edgar_logs_config.aws_upload_bucket_name is None or edgar_logs_config.aws_upload_bucket_name == "":
    log_entry("Required parameter 'edgar_logs_config.aws_upload_bucket_name' is missing: check the edgar_logs_config.py file")
    sys.exit()

log_entry("Using year: " + str(year))
log_entry("Upload bucket name is: " + edgar_logs_config.aws_upload_bucket_name)
...
s3 = boto3.resource('s3',
    aws_access_key_id=edgar_logs_config.aws_access_key_id,
    aws_secret_access_key=edgar_logs_config.aws_secret_access_key)


# TODO: write all the data and summaries to a file, and upload it to S3.
...
data = open(logfile_name, 'rb')
s3.Bucket(edgar_logs_config.aws_upload_bucket_name).put_object(Key=logfile_name, Body=data)
data.close()
