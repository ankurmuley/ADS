...
# Create logfile.
logfile_name = "edgar-logfile.txt"
...
if edgar_config.aws_access_key_id is None or edgar_config.aws_access_key_id == "":
    log_entry("Required parameter 'edgar_config.aws_access_key_id' is missing: check the edgar_config.py file")
    sys.exit()

if edgar_config.aws_secret_access_key is None or edgar_config.aws_secret_access_key == "":
    log_entry("Required parameter 'edgar_config.aws_secret_access_key' is missing: check the edgar_config.py file")
    sys.exit()

if edgar_config.aws_upload_bucket_name is None or edgar_config.aws_upload_bucket_name == "":
    log_entry("Required parameter 'edgar_config.aws_upload_bucket_name' is missing: check the edgar_config.py file")
    sys.exit()

log_entry("Using CIK: " + str(cik))
log_entry("Using Accession number: " + str(accession))
log_entry("Upload bucket name is: " + edgar_config.aws_upload_bucket_name)
...
s3 = boto3.resource('s3',
    aws_access_key_id=edgar_config.aws_access_key_id,
    aws_secret_access_key=edgar_config.aws_secret_access_key)


# TODO: ZIP the table files, and upload the ZIP archive to S3.
...
s3.Bucket(edgar_config.aws_upload_bucket_name).put_object(Key=logfile_name, Body=data)
data.close()
